...
#
# Source lives in WSL at /home/alex/midway-univiewer
# SDL2 shared with bddview/imgtool in %LOCALAPPDATA%\midway-build\deps

param(
    [string]$SourceDir  = "\\wsl.localhost\Ubuntu\home\alex\midway-univiewer",
    [string]$BuildRoot  = "$env:LOCALAPPDATA\univiewer-build",
...
$batFile = "$env:TEMP\build_univiewer.bat"
[System.IO.File]::WriteAllLines($batFile, $lines, [System.Text.Encoding]::ASCII)
& cmd.exe /c $batFile
$exitCode = $LASTEXITCODE

if ($exitCode -eq 0) {
    $exe = "$buildDir\Release\univiewer.exe"
    Write-Host ""
    Write-Host "*** Build succeeded ***" -ForegroundColor Green
    Write-Host "EXE: $exe" -ForegroundColor Green
    $sdl2Dll = "$sdl2Root\lib\x64\SDL2.dll"
